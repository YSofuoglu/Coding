...
A string representing the correct date in the format mm-dd-yyyy, where mm is the number of month (1-based, i.e. 01 for
January, 02 for February and so on), dd is the day, and yyyy is the year.
...
def f(birthdayDate):
    ...
    return next(yrs - y for yrs in range(y + 1, 2113)
                if (calendar.isleap(yrs) or d != 29 or m != 2)
                and datetime.date(yrs, m, d).weekday() == datetime.date(y, m, d).weekday())
